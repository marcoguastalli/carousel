...
**Docker distribution:** [Dockerfile](Dockerfile) builds the app shell and serves it with nginx; [.github/workflows/docker-publish.yml](.github/workflows/docker-publish.yml) publishes it to GHCR (`ghcr.io/marcoguastalli/carousel`). Images are **not** baked in (excluded via [.dockerignore](.dockerignore)) — they are mounted at runtime as a volume on `/usr/share/nginx/html/images`. Because the Vite manifest plugin only runs at build time, [docker/40-generate-manifest.sh](docker/40-generate-manifest.sh) (run by nginx's `/docker-entrypoint.d/` on startup) regenerates the manifest from the mounted volume, and [docker/default.conf](docker/default.conf) makes nginx serve `/images/manifest.json` from that generated file (kept outside the volume so it can be mounted read-only).
